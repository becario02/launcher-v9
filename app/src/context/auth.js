...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (authCookie && isAdminPath && profileName) {
      ...
      if (
        ...
        !profileName.includes("ADVAN") &&
        !(isUserCustomer && isAllowedPath)
      ) {
        router.push("/");
        return;
      }
  ...
  const login = (userData) => {
    ...
    if (userData.profileName) {
      Cookies.set("profileName", userData.profileName, {
        expires: 7,
        path: "/",
      });
    }
  ...
  const logout = () => {
    ...
    ["auth", "tabs", "activeTabId", "tabCounter", "profileName"].forEach(
      (cookieName) => {
