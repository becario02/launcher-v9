        ...
        <div className="relative bg-white dark:bg-[#1c1c24] rounded-2xl shadow-2xl max-w-lg w-full p-6 max-h-[80vh] overflow-auto">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-h4 font-semibold text-gray-900 dark:text-white">
              {type === 'custom' ? 'Permisos: Opciones personalizadas' : 'Permisos: Dashboards'}
            </h2>
          ...
          <div className="space-y-4 overflow-y-auto max-h-64 pr-1 mb-4">
            {isLoading ? (
              Array.from({ length: 6 }).map((_, index) => (
                <div key={index} className="flex items-center justify-between bg-gray-100 dark:bg-gray-800 p-3 rounded-lg animate-pulse">
              ...
              <div className="flex items-center justify-between bg-gray-100 dark:bg-gray-800 p-3 rounded-lg">
                <span className="text-sm text-gray-600 dark:text-gray-400 flex items-center gap-2">
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M4 6h16M4 10h16M4 14h16M4 18h16" />
                  </svg>
              ...
              options.map((item) => {
                ...
                return (
                  <div key={key} className="flex items-center justify-between bg-gray-100 dark:bg-gray-800 p-3 rounded-lg">
                    <span className="text-p text-gray-900 dark:text-white">
                      {item.textOption || item.title || 'Sin nombre'}
                    </span>
