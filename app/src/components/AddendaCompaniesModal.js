...
export default function AddendaCompaniesModal({ 
  isOpen, 
  onClose, 
  addenda,
  onSuccess
  ...
  const [addendaCompanies, setAddendaCompanies] = useState([]);
  ...
  // Cargar compañías de la addenda
  const fetchAddendaCompanies = async () => {
    ...
    try {
      ...
      if (response.data && response.data.statusCode === "200" && response.data.data) {
        // Extraer los idCompany de las compañías asignadas
        const assignedCompanyIds = response.data.data.map(c => c.idCompany);
        setAddendaCompanies(response.data.data);
        setInitialCompanyIds(assignedCompanyIds);
        setFormData({ companyIds: assignedCompanyIds });
      }
    } catch (error) {
      console.error('Error al cargar compañías de la addenda:', error);
      setAddendaCompanies([]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!addenda?.idAddenda) {
      console.error('No hay addenda seleccionada');
      return;
    }
    ...
    try {
      ...
      if (response.data.statusCode === "200") {
        if (onSuccess) {
          onSuccess({
            type: 'success',
            message: `Compañías actualizadas para la addenda ${addenda.name}`
          });
        }
      ...
      if (onSuccess) {
        onSuccess({
          type: 'error',
          message: 'Error al actualizar las compañías de la addenda'
        });
      }
    } finally {
      ...
      <div className="relative bg-white dark:bg-[#1C1C24] rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-[#2C2C38] flex-shrink-0">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">
            Gestionar Compañías - {addenda?.name}
          </h2>
          <button
            onClick={handleClose}
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="bg-gray-50 dark:bg-[#262631] rounded-lg p-4">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
                Gestiona qué compañías pueden utilizar esta addenda.
              </p>
              <div className="flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400">
                <span>Estructuras disponibles:</span>
                <div className="flex gap-1">
                  {addenda?.structureTravel && (
                    <span className="bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200 px-2 py-0.5 rounded text-xs">
                      Viaje
                    </span>
                  )}
                  ...
                  {!addenda?.structureTravel && !addenda?.structureAdditionalInvoice && !addenda?.structureCreditNote && (
                    <span className="text-gray-500 dark:text-gray-400">Ninguna</span>
                  )}
                </div>
              </div>
            </div>
